...
bool CRunDlg::DoExecute(const command * cmd)
{
	...
	const wchar_t * verb = hlp::is_key_down(VK_SHIFT) ? L"runas" : L"open";
	...
	if (ShellExecute(m_hWnd, verb, path.c_str(), param.c_str(), work_dir.c_str(), show_cmd) <= (HINSTANCE)32)
	{
...
//	m_background  = (HBITMAP)::LoadImage(NULL, L"d:\\anyplace\\aa.BMP", IMAGE_BITMAP, 0, 0, LR_DEFAULTSIZE | LR_LOADFROMFILE);
	...
	BOOL tmp;
	IDropTargetImpl::OnCreate(0, 0, 0, tmp);
...
LRESULT CRunDlg::OnNcMouseAction(UINT uMsg, WPARAM , LPARAM , BOOL& )
{
	...
	{
		// 在简洁模式时强制获取焦点
		show_window(true, g_c->gm.layout == cfg::ul_simple);
	}
	return 0;
}
