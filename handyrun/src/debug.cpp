...
const struct msg_map
{
	int msg_id;
	const wchar_t * msg_name;
} g_mm[] =
{
0x0000, L"WM_NULL",
0x0001, L"WM_CREATE",
...
0x0400, L"WM_USER",
0x8000, L"WM_APP",
0xFFFF, L"unknown message",
};

const wchar_t * dbg::get_msg_name(int msg_id)
{
	struct _hlp
	{
		static bool msg_less(const msg_map& mm1, const msg_map& mm2)
		{
			return mm1.msg_id < mm2.msg_id;
		}
	};
	const msg_map * mend = g_mm + sizeof(g_mm)/sizeof(g_mm[0]) - 1;
	msg_map m = {msg_id, NULL};
	const msg_map * p = std::lower_bound(g_mm, mend, m, _hlp::msg_less);
	return p->msg_name;
}